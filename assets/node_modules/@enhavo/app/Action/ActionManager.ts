...
export default class ActionManager
{
    ...
    private initializeActions(actions: ActionInterface[]): void
    {
        for (let i in actions) {
            let action = this.registry.getFactory(actions[i].component).createFromData(actions[i]);
            _.extend(actions[i], action);
            this.initializeActions(actions[i].subActions());
        }
    }
}
