...
export class NotFoundErrorHandler implements ErrorHandlerInterface
{
    constructor(
        ...
        return new Promise((resolve, reject) => {
            if (options.confirm) {
                this.uiManager.alert({
                    message: this.translator.trans('enhavo_app.error_404', {}, 'javascript')
                }).then(() => {
                ...
                this.flashMessenger.error(this.translator.trans('enhavo_app.error_404', {}, 'javascript'));
                if (options.terminate) {
                    this.frameManager.close(true);
                }
                resolve();
    ...
    {
        return transport.ok && transport.response.status === 404;
    }
}
