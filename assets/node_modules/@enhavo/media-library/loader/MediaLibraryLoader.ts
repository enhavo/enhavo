...
export default class MediaLibraryLoader extends AbstractLoader
{
    ...
    private initUpdateEvent(mediaType: MediaType)
    {
        this.eventDispatcher.on('updated', (event: UpdatedEvent) => {
            this.eventDispatcher.dispatch(new LoadDataEvent('MediaLibrary'))
    ...
    {
        let url = this.router.generate('enhavo_media_library_index', {
            multiple: mediaType.getConfig().multiple ? 1 : 0
        });
        let elements = this.findElements(mediaType.getElement(), '[data-file-MediaLibrary]');

        for(let element of elements) {
            $(element).click((event) => {
                event.preventDefault();
                this.view.open(url, 'MediaLibrary');
