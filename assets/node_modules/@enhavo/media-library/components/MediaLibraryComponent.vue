...
onMounted(() => {
    window.addEventListener('keydown', (e) => {
        if (e.key == 'Enter') {
            let isSearchFocus = jQuery(elements.searchInput).is(':focus');
            if (isSearchFocus) {
                search();
            }
        }
    ...
    jQuery(document).on('upload', function () {
        jQuery(elements.upload).trigger('click');
    });

    jQuery(elements.upload).fileupload({
    ...
    jQuery(document).on('dragover', (e) => {
        e.preventDefault();
        e.stopPropagation();
        mediaLibrary.showDropZone();
    });

    jQuery(elements.mediaLibrary).on('dragover', (e) => {
        e.preventDefault();
        e.stopPropagation();
        mediaLibrary.showDropZone();
    ...
    jQuery(document).on('dragleave', (e) => {
        if (jQuery(document).find('.app-view').length > 0 && jQuery(document).find('.app-view').find(e.target).length > 0) return;
        e.preventDefault();
        e.stopPropagation();
        mediaLibrary.hideDropZone();
    });

    jQuery(elements.mediaLibrary).on('dragleave', (e) => {
        e.preventDefault();
        e.stopPropagation();
        mediaLibrary.hideDropZoneActive();
    });

    jQuery(document).on('drop', (e) => {
        e.preventDefault();
        e.stopPropagation();
        mediaLibrary.hideDropZone();
        mediaLibrary.hideDropZoneActive();
