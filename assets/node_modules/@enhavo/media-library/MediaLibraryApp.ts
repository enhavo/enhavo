...
export default class MediaLibraryApp
{
    ...
    public constructor(data: any, eventDispatcher: EventDispatcher, view: View, actionManager: ActionManager, flashMessenger: FlashMessenger, router: Router, componentRegistry: ComponentRegistryInterface, translator: Translator)
    {
    ...
    private add()
    {
        ...
        for(let image of this.data.selectedItems) {
            calls.push((callback) => {
                axios
                    .get(this.router.generate('enhavo_media_library_add', {id: image.id}))
                    .then(response => {
    ...
    public open(item: Item): void
    {
        if (this.data.mode === 'edit') {
            this.activateItem(item).then(() => {
                let url = this.router.generate('enhavo_media_library_update', {id: item.id, view_id: this.view.getId()});
    ...
    {
        this.loading();
        let url = this.router.generate('enhavo_media_library_content_types');
