...
export default class PolyCollectionType extends FormType
{
    ...
    public addItem(type: string, button: PolyCollectionItemAddButton)
    {
        ...
        jQuery(form.getElement()).after(newButton.getElement());
        this.setOrder();
    }
    ...
    private createItem(type : string): HTMLElement
    {
        ...
        let $item = jQuery(jQuery.parseHTML(itemTemplate));
    ...
    private createAddButton() : PolyCollectionItemAddButton
    {
        ...
        let element = jQuery.parseHTML(html)[0];
    ...
    public moveItemUp(item: PolyCollectionItem, callback: () => void = function() {})
    {
        ...
        if (index > 0) { // is not first element
            let buttonToMove = jQuery(this.$container.children('[data-poly-collection-add-button]').get(index + 1));
            let buttonTarget = jQuery(this.$container.children('[data-poly-collection-add-button]').get(index - 1));
            let domElementToMove = jQuery(item.getElement());
    ...
    public moveItemDown(item: PolyCollectionItem, callback: () => void = function() {})
    {
        ...
        if (index < (size - 1)) { // is not last element
            let buttonToMove = jQuery(this.$container.children('[data-poly-collection-add-button]').get(index + 1));
            let buttonTarget = jQuery(this.$container.children('[data-poly-collection-add-button]').get(index + 2));
            let domElementToMove = jQuery(item.getElement());
    ...
    {
        jQuery(item.getElement()).next().remove();
        jQuery(item.getElement()).css({opacity:0,transition: 'opacity 550ms'}).slideUp(350,function() {
            this.remove();
        });
