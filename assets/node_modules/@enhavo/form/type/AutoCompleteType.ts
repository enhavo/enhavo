...
export default class AutoCompleteType extends FormType
{
    ...
    private initEditFields(route:string)
    {
        ...
        let self = this;
        $items.each(function() {
            if(typeof jQuery(this).data('initialized') == "undefined"){
                jQuery(this).data('initialized', 1);
                jQuery(this).click((event:any) => {
                    event.preventDefault();
                    let id = jQuery(event.target).data('auto-complete-edit');
                    // open edit view
