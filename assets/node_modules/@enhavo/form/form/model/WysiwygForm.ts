...
export class WysiwygForm extends Form
{
    ...
    public initWysiwyg()
    {
        // if init is called to early after a remove was triggered, tinymce will not initialize, so we have to wait a few milliseconds
        window.setTimeout(() => {
    ...
    public removeWysiwyg()
    {
        if (this.editorElement) {
            tinymce.remove('#'+this.editorElement.id);
        }
    }
    ...
    {
        // because this id will be used in a querySelectorAll later from tinymce, the id must start with a letter and not a number
        return 'bxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
